...
<jsp:useBean id="listCaseRep_ByCompositeQuery" scope="request"
	type="java.util.List" />
<%
					...
					<FORM METHOD="post"
						...
						<div class="col-sm-2">
							<div class="form-group">
								<label>檢舉案例編號</label> <input type="text" class="form-control"
									name="crepno" value="">
							</div>
						</div>
						...
						<div class="col-sm-2">
							<div class="form-group">
								<label>受檢舉合購編號</label> <input type="text" class="form-control"
									name="susno" value="">
							</div>
						</div>
						...
						<div class="col-sm-2">
							<div class="form-group">
								<label>合購案編號</label> <input type="text" name="repcaseno"
						...
						<div class="col-sm-2">
							<div class="form-group">
								...
								<input type="hidden" name="action"
									value="listCaseRep_ByCompositeQuery">
							</div>
					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover"
							...
							<tbody>
								...
								<c:forEach var="caseRepVO"
									items="${listCaseRep_ByCompositeQuery}">
									<tr class="odd gradeX">
										...
										<td><c:forEach var="caseVO" items="${caselist}">
												<c:if test="${caseVO.caseno == caseRepVO.repcaseno}">
													<a href="<%=request.getContextPath()%>/cases/cases.do?action=caseDetail&caseno=${caseVO.caseno}">${caseVO.title}(${caseVO.caseno})</a>
												</c:if>
											</c:forEach></td>
										<td>${caseRepVO.creprsn}</td>
										<td>
											<FORM METHOD="post"
												action="<%=request.getContextPath()%>/back/cases/CancelCase.do">
												<c:forEach var="caseVO" items="${caselist}">
													<c:if test="${caseVO.caseno == caseRepVO.repcaseno}">
														<c:if
															test="${caseVO.status == 1 or caseVO.status == 2 or caseVO.status == 3}">
															<input type="submit" class="btn btn-primary" value="下架">
														</c:if>
														<c:if
															test="${caseVO.status == 0 or caseVO.status == 4 or caseVO.status == 5 or caseVO.status == 6}">
												...
												<input type="hidden" name="action"
													value="CancelCase_from_back"> <input type="hidden"
													name="requestURL" value="<%=request.getServletPath()%>">
												<input type="hidden" name="repcaseno"
													value="${caseRepVO.repcaseno}">
											</FORM>
										</td>
										<td>
											<FORM METHOD="post"
												...
												<input type="hidden" name="crepno"
													value="${caseRepVO.crepno}"> <input type="hidden"
													name="requestURL" value="<%=request.getServletPath()%>">
