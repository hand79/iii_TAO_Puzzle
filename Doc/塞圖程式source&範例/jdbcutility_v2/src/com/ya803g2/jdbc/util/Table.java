...
public class Table {
	...
	public String getDumpPicStmt(){
		StringBuilder sql = new StringBuilder("SELECT ");
		sql.append(pkColumn);
		Set<String> picColNames = blob.keySet(); 
		for(String col: picColNames){
			sql.append(", ").append(col);
			sql.append(", ").append(blob.get(col));
		}
		sql.append(" FROM ").append(name).append(" ORDER BY ").append(pkColumn);
		return sql.toString();
	}
